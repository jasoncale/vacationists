...
Vacationists.Player = (function() {
    ...
    var initTracks = function(trackSel) {
        $(trackSel).each(function() {
            if ($(this).attr('href')) {
                ...
                // make player full volume
                player.setVolume(100);

                // remove style attributes from player
                $(player.container).parent().attr('style', 'position: relative');
    ...
    broadcast.registerCallback('trackPlay',
    function(track) {
        currentTrack = track;
        stopAllTracks(track);
    });
    broadcast.registerCallback('trackComplete',
    function(track) {
        playNextTrack(track);
    });

    var _registerCallback = function(name, callback) {
        broadcast.registerCallback(name, callback);
    }
    var _playTrackId = function(trackId) {
        jwplayer(trackId).play();
    }
    var _currentTrackNumber = function() {
        return tracks.indexOf(currentTrack) + 1;
    }
...
Vacationists.Layout = (function() {
    ...
    var repositionImages = function() {
        ...
        $('.images').each(function() {
            var track = $(this).attr('class').replace(/images/, '');
            track = $.trim(track);
            if (track.length > 0) {
                if (imagePositions[track] == undefined) {
                    ...
                    $(this).find('.image').each(function() {
                        ...
                        imagePositions[track][$(this).attr('id')] = {
                            left: $(this).css('background-position-x')
                        };
                    });
                }
    ...
    Vacationists.Player.registerCallback('trackPlay',
    function(track) {
        updateTrackStatus(track);
    });
    Vacationists.Player.registerCallback('trackPause',
    function(track) {
        updateTrackStatus(track);
    });
    Vacationists.Player.registerCallback('trackComplete',
    function(track) {
        updateTrackStatus(track);
    });
    Vacationists.Player.registerCallback('trackStop',
    function(track) {
        updateTrackStatus(track);
    });
